...
- StageSelect 그림자 계약서는 7종 적성 기록, 조각 진행, 계약, 단일 장착과 해제를 제공한다. Android 자동 시스템 흐름과 1080x2400 렌더 캡처는 통과했지만 실기기 빠른 연속 탭, 스크롤 관성과 시스템 글꼴 확대는 추가 확인이 필요하다.
- 그림자 계약 패시브와 계약 비용은 1차 밸런스다. 펫 전용 애니메이션, 전투 동행 모델과 개별 성장 시스템은 아직 포함하지 않는다.
...
- UpgradeScene의 2x2 Android Portrait 카드, 구매 피드백, 씬 재진입 저장은 1080x2400 에뮬레이터에서 통과했다. 실기기 컷아웃, 시스템 글꼴 확대, 빠른 연속 탭은 아직 검증하지 않았다.
